...
class Graph():
    ...
    def get_alias_edge(self, src, dst):
        ...
        unnormalized_probs = []
        for dst_nbr in list(G.neighbors(dst)):
            if dst_nbr == src:
    ...
    def preprocess_transition_probs(self):
        ...
        alias_nodes = {}
        for node in G.nodes():
            tmp = [G[node][nbr]['weight'] for nbr in G.neighbors(node)]
            # print(tmp)
            # print(node)
            # print(G.neighbors(node))
            # # print([n for n in G.neighbors(node)])
            # print(sorted(G.neighbors(node)))
            # print(len(sorted(G.neighbors(node))))
            # print(type(sorted(G.neighbors(node))))
            # print(list(G.neighbors(node)))
            # print(len(G.neighbors(node)))
            # str_neighbors = [str(n) for n in G.neighbors(node)]
            # unnormalized_probs = [G[node][nbr]['weight'] for nbr in sorted(G.neighbors(node))]
            unnormalized_probs = [G[node][nbr]['weight'] for nbr in list(G.neighbors(node))]
...
def generate_random_walks(input, num_walks, walk_length):
    ...
    G = Graph(input, is_directed=False, p=1, q=1)  #DeepWalk
